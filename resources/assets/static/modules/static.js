...
;!function (win) {
   ...
   Admin.prototype.dateTime = function(elem,type,value,min,max,is = false,format = ''){
     ...
     * box 最后填充地址
     */
    Admin.prototype.uploadImgBox = function (elem,box){
      layui.upload.render({
          elem: elem
          ...
          , done: function (res) {
              ...
              if (res.status == 1) {
                  if(length < num){
                      $(box).append('<li><img src="' + res.info[0].path + '" /><input type="hidden" name="'+name+'" value="'+ res.info[0].path + '"><span hidden="" class="img-delete" style="display: none;"><i class="icon-shanchu iconfont"></i></span></li>')
